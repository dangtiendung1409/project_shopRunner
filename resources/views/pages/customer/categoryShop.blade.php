            ...
            <div class="row">
                ...
                <div class="col-lg-9">
                    ...
                    <form style="display: flex" action="{{ url("/category/") }}" method="get">
                        ...
                        <div class="input-group input-group-sm mr-2" style="width: 150px; margin-left: 5px;">
                            <select value="{{ app("request")->input("brand_id") }}" style="height: 45px;" name="brand_id" class="form-control">
                                <option value="0">Branding</option>
                                @foreach($brands as $item)
                                    <option value="{{ $item->id }}" {{ app("request")->input("brand_id") == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
